...
namespace RosterLib
{
	public class DbfPredictionQueryMaster : IGetPredictions
	{
			...
			string gameCode)
		{
			...
			var ds = Utility.TflWs.GetPrediction(
				method,
				season,
				week,
				gameCode);
			...
			var game = new NFLGame( $"{season}:{week}-{gameCode}" );
			...
			var result = new NFLResult(
				home: game.HomeTeam,
				homePts: prediction.HomeScore,
				away: game.AwayTeam,
				awayPts: prediction.AwayScore)
			{
		...
		{
			return Int32.Parse( 
				ds.Tables[ 0 ].Rows[ 0 ][ fieldName ].ToString() );
		}
	}
}
