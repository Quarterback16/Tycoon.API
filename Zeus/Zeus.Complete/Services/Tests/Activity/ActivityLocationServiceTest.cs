...
{
    [TestClass]
    public class ActivityLocationServiceTest
    {
        ...
        #region Activity location test
        /// <summary>
        /// Test valid activity location list
        /// </summary>
        [TestMethod]
        public void ListLocations_Valid()
        {
            ...
            var response = new ACTLocSearchResponse() { Locations = list.ToArray() };
        ...
        public void GetLocation_Valid()
        {
            ...
            //a.request
            long activityId = 123123;
            long locationSeqNumber = 12;

            var response = ActivityTestDataHelper.CreateDummyACTGetLocDetailsResponse();

            mockActivityHostLocationtWcf.Setup(m => m.GetLocationDetails(It.IsAny<ACTGetLocDetailsRequest>())).Returns(response);

            //2. Exec service
            var result = SystemUnderTest().GetLocation(activityId, locationSeqNumber);

            //3. Verification
            Assert.AreEqual(123, result.HostId);
