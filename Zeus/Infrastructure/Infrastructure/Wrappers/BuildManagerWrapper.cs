...
namespace Employment.Web.Mvc.Infrastructure.Wrappers
{
    ...
    public class BuildManagerWrapper : IBuildManager
    {
        ...
        public virtual IEnumerable<Assembly> Assemblies
        {
            ...
            get
            {
                return referencedAssemblies ?? 
                    (referencedAssemblies = BuildManager.GetReferencedAssemblies().Cast<Assembly>()
                    .Where(assembly => !assembly.GlobalAssemblyCache && assembly.FullName.StartsWith("Employment.Web.Mvc",StringComparison.Ordinal)).ToList());
            }
        }
        ...
        public Type ResolveType(string fullTypeName)
        {
            ...
            if (!string.IsNullOrEmpty(fullTypeName) && fullTypeName.IndexOf(splitter,StringComparison.OrdinalIgnoreCase)>=0)
            {
                ...
                allReferencedAssemblies = allReferencedAssemblies ?? 
                    BuildManager.GetReferencedAssemblies().Cast<Assembly>();

                var assemblyWithType = allReferencedAssemblies
