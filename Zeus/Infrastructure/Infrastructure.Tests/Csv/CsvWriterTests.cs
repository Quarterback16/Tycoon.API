...
namespace Employment.Web.Mvc.Infrastructure.Tests.Csv
{
    ...
    public class CsvWriterTests
    {
        ...
        {
            var record = new TestInvariantDateTime {dateColumn = new DateTime(2012,12,1)};
            var stream = new MemoryStream(); 
            var writer = new StreamWriter(stream) { AutoFlush = true };
            var csv = new CsvWriter(writer, new CsvConfiguration { UseInvariantCulture = false }); 
            csv.WriteRecord(record); 
            stream.Position = 0; 
            var reader = new StreamReader(stream); 
            var csvFile = reader.ReadToEnd(); 
            var expected = "dateColumn\r\n";
            expected += "01/12/2012\r\n";
        ...
        private class TestInvariantDateTime
        {
            [CsvField(Format = "{0:dd/MM/yyy}")]
