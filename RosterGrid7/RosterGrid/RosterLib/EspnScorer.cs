	...
	public class EspnScorer : IRatePlayers
	{
		...
		public Decimal RatePlayer(NFLPlayer plyr, NFLWeek week )
		{
         ...
         plyr.Points += PointsFor(plyr, 4, Constants.K_SCORE_TD_PASS, id: "2");
			if (AnnounceIt)
			  Utility.Announce( string.Format( formatStr, plyr.PlayerName, plyr.Points, "Tdp" ) );
			...
			if (AnnounceIt)
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForPATPasses, "PAT passes" ) );
         ...
         var ptsForYDp = PointsForStats(plyr: plyr, increment: 1, forStatType: Constants.K_STATCODE_PASSING_YARDS, divisor: 25.0M);
			...
			if ( AnnounceIt )
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForYDp, "YDp" ) );
         ...
         var ptsForInts = PointsForStats(plyr: plyr, increment: -2, forStatType: Constants.K_STATCODE_INTERCEPTIONS_THROWN, divisor: 1.0M);
			...
			if ( AnnounceIt )
			   Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForInts, "Interceptions" ) );
			...
			if ( AnnounceIt )
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForTDcatches, "TD catches" ) );

			var ptsForPATcatches = PointsFor( plyr, 2, Constants.K_SCORE_PAT_PASS, id :"1" );
			plyr.Points += ptsForPATcatches;

			if ( AnnounceIt )
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForPATcatches, "PAT catches" ) );
			...
			if ( AnnounceIt )
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForYDs, "YDc" ) );
			...
			if ( AnnounceIt )
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForTDruns, "TD runs" ) );

			var ptsForPaTruns = PointsFor( plyr, 2, Constants.K_SCORE_PAT_RUN, id :"1" );
			plyr.Points += ptsForPaTruns;

			if ( AnnounceIt )
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForPaTruns, "PAT runs" ) );
			...
			if ( AnnounceIt )
				Utility.Announce( string.Format( formatStr, plyr.PlayerName, ptsForYDr, "YDr" ) );
		...
		private int PointsFor(NFLPlayer plyr, int increment, string forScoreType, string id )
		{
			...
			switch (forScoreType)
			{
				...
				default:
					Utility.Announce(string.Format("Unknown score type {0}", forScoreType));
					break;
			}
			return points;
		}
