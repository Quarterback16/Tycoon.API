...
namespace RosterLib
{
   ...
   public class SimpleTDrPredictor : IPredictTDr
   {
      ...
      public Int32 PredictTDr(NFLPlayer plyr, string season, int week)
      {
         ...
         {
            if (plyr.CurrTeam.Ratings.Equals("CCCCCC")) plyr.CurrTeam.SetRecord(season);

            //  who are the opponents
            NflTeam opponent = plyr.CurrTeam.OpponentFor(season, week);
            if (opponent != null)
            {
               if (opponent.Ratings.Equals("CCCCCC")) opponent.SetRecord(season);  // Incase not initialised
               //  not on a bye
               tDr = 1;
