...
namespace RosterLib
{
	public class ScoreTally
	{
		...
		public void Render()
		{
			var str = new SimpleTableReport( string.Format( "Score Grid {0}-{1}", Season.Year, ScopeInFocus ) ) { ReportHeader = ScopeInFocus };
		...
		public string FileName()
		{
			if ( UsingPredictions )
				return string.Format( "{0}{1}//projections//ScoreGrid-{2}.htm",
					Utility.OutputDirectory(), Season.Year, ScopeInFocus );
			else
		...
		private void WriteGameCountLine( DataTable dt )
		{
			...
			{
				var weekNo = string.Format( "WK{0}", week.Week );
				var preds = GetPredictions( Season.Year, week.Week );
				var nInWeek = 0;
		...
		private DataSet GetPredictions( string season, string week )
		{
