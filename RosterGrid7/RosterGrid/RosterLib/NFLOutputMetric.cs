namespace RosterLib
{
	...
	public class NFLOutputMetric
	{
      ...
      private static string TeamUrl( string teamCode )
      {
		...
		private static string ProjectionUrl( int projectedSpread, string opponent, bool isHome, string teamCode, 
			int week )
		{
			...
			if ( projectedSpread != 0 )
			{
				...
				url = string.Format( "<a href =\"./gameprojections/{4:0#}-{1}@{2}.htm\">{3}</a>", 
					Utility.CurrentSeason(), awayTeam, homeTeam, projectedSpread, week );
			}
			return url;
		}
	}
